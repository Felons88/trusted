...
-- Trusted Mobile Detailing - Full Database Schema
...
CREATE INDEX idx_two_factor_auth_enabled ON two_factor_auth(enabled, user_id);
...
CREATE INDEX idx_payment_details_payment_intent_id ON public.payment_details(stripe_payment_intent_id);
CREATE INDEX idx_fraud_detection_payment_attempt_id ON public.fraud_detection(payment_attempt_id);
CREATE INDEX idx_fraud_detection_fraud_score ON public.fraud_detection(fraud_score);
CREATE INDEX idx_fraud_detection_risk_level ON public.fraud_detection(risk_level);
CREATE INDEX idx_payment_status_updates_payment_attempt_id ON public.payment_status_updates(payment_attempt_id);
...
-- Function to auto-promote admin users
...
BEGIN
  ...
  IF NEW.email LIKE 'daniel2002@%' THEN
    INSERT INTO profiles (id, email, full_name, role)
...
RETURNS TRIGGER AS $$
BEGIN
  -- Create a basic profile for new users (if not already created by auto_promote_admin)
  INSERT INTO profiles (id, email, full_name, role)
...
CREATE POLICY "Users can view their own payment details" ON public.payment_details FOR SELECT USING (
  EXISTS (
    ...
    JOIN clients i ON i.id = pa.client_id
...
SELECT 'DATABASE SETUP COMPLETE - All tables, functions, and policies created!' as status;
