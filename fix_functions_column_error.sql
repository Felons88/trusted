-- Fix the column error in payment functions
-- This addresses the "payment_attempt_id_param does not exist" error

-- First drop all functions again
DO $$
DECLARE
    func_record RECORD;
BEGIN
...
-- Recreate record_payment_attempt with fixed INSERT
...
BEGIN
  ...
  -- Fixed INSERT with proper column names and created_at
...
-- Recreate record_payment_details
...
-- Recreate update_payment_attempt_details with fixed INSERT
...
BEGIN
  ...
  -- Fixed INSERT - using VALUES instead of SELECT to avoid column reference issues
  INSERT INTO public.payment_status_updates (
    payment_attempt_id, old_status, new_status, reason, created_at
  )
  SELECT 
