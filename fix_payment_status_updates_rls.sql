-- Fix RLS issues for payment_status_updates table
-- Add proper policies and ensure functions can bypass RLS

-- First, add admin access policy for payment_status_updates if not exists
...
-- Add policy for users to insert their own status updates
...
-- Now recreate all functions with SECURITY DEFINER to bypass RLS completely
DO $$
DECLARE
    func_record RECORD;
BEGIN
...
-- record_payment_attempt with SECURITY DEFINER
...
-- record_payment_details with SECURITY DEFINER
...
-- update_payment_attempt_details with SECURITY DEFINER
